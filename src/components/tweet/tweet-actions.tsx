...
const pinModalData: Readonly<PinModalData[]> = [
  {
    title: 'Pin Claw to from profile?',
    description:
      'This will appear at the top of your profile and replace any previously pinned Claw.',
    mainBtnLabel: 'Pin'
  },
  {
    title: 'Unpin Claw from profile?',
    description:
...
export function TweetActions({
  ...
  const handleRemove = async (): Promise<void> => {
    ...
    toast.success(
      `${isInAdminControl ? `@${username}'s` : 'Your'} Claw was deleted`
    );
  ...
  const handlePin = async (): Promise<void> => {
    ...
    toast.success(
      `Your Claw was ${tweetIsPinned ? 'unpinned' : 'pinned'} to your profile`
    );
    pinCloseModal();
  };
